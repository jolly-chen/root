...
/**
 \file ROOT/RDF/GPUFillHelper.hxx
 \ingroup dataframe
...
#ifndef ROOT_GPUFILLHELPER
#define ROOT_GPUFILLHELPER
...
template <template <typename, unsigned int ...> class RHnType, typename HIST = Hist_t>
class R__CLING_PTRCHECK(off) GPUFillHelper : public RActionImpl<GPUFillHelper<RHnType, HIST>> {
   static constexpr size_t dim = getHistDim((HIST *)nullptr);

   std::vector<HIST *> fObjects;
   std::vector<RHnType<decltype(getHistType((HIST *)nullptr)), dim> *> fGPUHist;
...
public:
   GPUFillHelper(GPUFillHelper &&) = default;
   GPUFillHelper(const GPUFillHelper &) = delete;

   // Initialize fGPUHist
   inline void init_GPU(HIST *obj, int i)
   {
      if (getenv("DBG"))
         printf("Init GPU hist %d\n", i);
      ...
      fGPUHist[i] = new RHnType<decltype(getHistType((HIST *)nullptr)), dim>(ncells, xlow, xhigh, binEdges.data());
   }

   GPUFillHelper(const std::shared_ptr<HIST> &h, const unsigned int nSlots)
      : fObjects(nSlots, nullptr), fGPUHist(nSlots, nullptr)
   {
      fObjects[0] = h.get();
      init_GPU(fObjects[0], 0);

      // Initialize all other slots
      for (unsigned int i = 1; i < nSlots; ++i) {
         fObjects[i] = new HIST(*fObjects[0]);
         UnsetDirectoryIfPossible(fObjects[i]);
         init_GPU(fObjects[i], i);
      }
   }
   ...
   void Finalize()
   {
      ...
      for (unsigned int i = 0; i < fObjects.size(); ++i) {
         HIST *h = fObjects[i];
         fGPUHist[i]->RetrieveResults(h->GetArray(), stats);
         h->SetStatsData(stats);
         h->SetEntries(fGPUHist[i]->GetEntries());
